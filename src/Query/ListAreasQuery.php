...
class ListAreasQuery
{
     ...
     * @var array|string[]
     */
    public static array $validSorts = ['id', 'name', 'state', 'supplyArea', 'createdAt', 'updatedAt'];
    ...
    public function getResults(int $page = 1, int $perPage = 20, string $sortBy = 'name', string $orderBy = 'asc', string $search = ''): Paginator
    {
        ...
        $qb->select('da.id, da.name, da.createdAt, da.updatedAt, s.id as state_id, s.name as state, sa.id as supplyArea_id, sa.name as supplyArea')
